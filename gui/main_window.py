...
from sdr.pluto_device import PlutoSDRDevice
from sdr.pipeline import SDRPipeline
from gui.widgets.sdr_workspace_widget import SDRWorkspaceWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # --------------------------------------------------------------
        # SDR source (real PlutoSDR, falling back to the simulator)
        # --------------------------------------------------------------
        self.sdr = self._create_sdr_device()

        self.sdr_pipeline = SDRPipeline(
            device=self.sdr,
            buffer_capacity=65_536,
            acquisition_block_size=2048,
            fft_size=self.SDR_FFT_SIZE,
        )
    ...
    def _create_sdr_device(self):
        """
        Build the SDR source used by the pipeline.

        Uses a real ADALM-PLUTO when USE_PLUTO_SDR (or the
        ORBITLAB_SDR_BACKEND=pluto environment variable) requests it,
        and falls back to SDRSimulator if pyadi-iio is missing or the
        device cannot be reached — the rest of the app doesn't care
        which one it got, since both implement the same SDRDevice
        interface.
        """
        import os

        backend = os.environ.get(
            "ORBITLAB_SDR_BACKEND",
            "pluto" if self.USE_PLUTO_SDR else "simulator",
        ).strip().lower()
    ...
    def update_telemetry(self, state, pass_info):
        ...
        self.elevation_label.setText(
            f"{state.elevation_deg:.2f}Â°"
        )
        self.azimuth_label.setText(
            f"{state.azimuth_deg:.2f}Â°"
        )
        ...
        self.lat_label.setText(
            f"{state.sat_lat_deg:.2f}Â°"
        )
        self.lon_label.setText(
            f"{state.sat_lon_deg:.2f}Â°"
        )
        ...
        self.maxpass_label.setText(
            ...
            f"({pass_info.max_elevation_deg:.1f}Â°)"
        )
        self.los_label.setText(
            pass_info.los_time
        )
